...
- **`SECONDARY_PERSONAS`** — `SECONDARY_PERSONAS[personaId]` → array of adjacent/secondary personas who interact with that primary persona's world but aren't the primary actor. Each entry: `role`, `archetype`, `orgLevel`, `relationship` (prose describing how they relate to the primary persona), plus the same `fixed[]`/`dynamic[]`/`structural[]` triad. Rendered as `.sec-persona-tile` cards directly on the page (`buildSecondaryPersonaTiles()`, `#sec-tiles-<personaId>`) below each primary persona's card — no longer inside `openPersonaModal()`. Each tile opens its own dialog (`openSecondaryPersonaModal()`), reusing the same `persona-modal` shell as the primary persona dialog but labeled "Secondary Persona."

**No names, anywhere (2026-07-16):** every persona — primary, vertical variant, and secondary — is identified by role/job title + archetype (+ vertical for primaries/vertical variants, + org-chart level for secondaries) only. `PERSONAS_DETAIL.role`/`VERTICAL_ATTRS[..].persona` used to be `"Name · Job Title"`; now they're just the title. `quoteSrc` (primary persona quote attribution) and `name` (`SECONDARY_PERSONAS` entries, `PERSONAS_J` entries) were removed as fields entirely rather than repurposed — don't reintroduce them. Cross-references between personas inside `fixed[]`/`dynamic[]`/`structural[]` prose (e.g. "Bookings Manager (Bettie)") were rewritten to drop the parenthetical name, relying on the role name alone (e.g. "the Bookings Manager") since it's already unambiguous. This does **not** apply to the historical Session Log in `CLAUDE.md`, which keeps names as a historical record of past sessions.

**To add a new vertical persona variant:** add a keyed entry to `VERTICAL_ATTRS[personaId]`, titled by role only.
**To add a new secondary persona:** push a new object into the `SECONDARY_PERSONAS[personaId]` array — `role` + `orgLevel` + `archetype` must be enough to distinguish it from every other entry in that array, since there's no name to fall back on.

**Secondary personas are org-chart level, not vertical, as of 2026-07-23:** `SECONDARY_PERSONAS[personaId][i].orgLevel` replaced the earlier `vertical` field — secondary personas are no longer tagged by vertical at all (that context, where relevant, lives in the prose instead). `orgLevel` is one of: `Chief`, `VP`, `Director`, `Manager`, `Individual Contributor`, `External/Contractor`, `Entry Level`. This is a different axis than `VERTICAL_ATTRS`/primary-persona verticals, which are untouched. Rendered in the same UI slot that used to show vertical (`.sec-persona-tile-arch` on the tile, `.sec-persona-tag` in the modal) — no markup changes, just what field feeds it.

**How to classify `orgLevel` (added 2026-07-23, after research into real venue org charts):** industry research confirmed that job *titles* are not a reliable proxy for organizational level — IAVM's own competency standard documents the same mid-level job being titled "supervisor" at a large venue and "senior manager" at a small one, and a title containing the word "Chief" (e.g. a museum's Chief Operating Officer) can still be the #2 position, not the top one. Classify by **scope of authority + reporting relationship + employment relationship**, never by matching the title string:

- **Chief** — ultimate decision-making authority for the entire venue/institution, across every functional area. Nothing above them at this venue.
- **VP** — oversees a broad, multi-functional swath spanning several of the map's Areas at once (e.g. finance + facilities + security + operations together), reporting directly to the Chief. Sets direction for a *swath*, not the whole venue; not the final authority.
- **Director** — owns one functional domain end-to-end (maps to one Area) — sets that domain's policy/standards and is accountable for its outcomes. Domain-wide accountability for exactly one Area.
- **Manager** — runs a specific team, shift, or sub-function within a domain — has direct reports doing the operational work. Implements policy rather than setting it; bounded scope.
- **Individual Contributor** — executes a defined scope of specialized work independently, may coordinate across teams but has no direct reports.
- **Entry Level** — front-line, task-execution work with the least autonomy — follows assigned instructions. Least specialized/newest tier, often physical/on-the-ground.
- **External/Contractor** — not an employee of the venue at all — a vendor, agency staff member, or an entire outsourced function (e.g. catering run end-to-end by a contracted operator, not just individually contracted staff). Sits outside the venue's own reporting chain regardless of seniority within their own employer.

When a role's literal title conflicts with this rubric (the museum COO example above), the rubric wins — note the discrepancy in that persona's own `structural[]` text rather than silently picking one or the other, so the reasoning is visible to future contributors.
